...
export default function HelpCenterPage() {
    return (
        <main className="pt-24 bg-white min-h-screen">
            {/* Hero Search */}
            <section className="bg-gray-900 text-white py-24 px-8 relative overflow-hidden">
                ...
                <div className="max-w-4xl mx-auto text-center relative z-10">
                    ...
                    <div className="relative max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700 delay-200">
                        <input
                            ...
                            className="w-full pl-14 pr-6 py-5 rounded-xl border-none shadow-2xl shadow-black/50 text-gray-900 placeholder:text-gray-500 focus:ring-2 focus:ring-primary-500 transition-all outline-none text-lg"
                        />
            ...
            <section className="max-w-7xl mx-auto px-8 py-24">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {categories.map((cat, idx) => (
                        <div key={idx} className="group p-8 rounded-[2rem] border border-gray-100 bg-white hover:shadow-xl hover:shadow-gray-200/50 hover:border-gray-200 transition-all duration-300">
                            <div className="w-12 h-12 rounded-2xl bg-gray-50 flex items-center justify-center text-gray-900 mb-6 group-hover:scale-110 group-hover:bg-primary-50 group-hover:text-primary-600 transition-all">
                                <cat.icon className="w-6 h-6" />
                            </div>
                            <h3 className="text-xl font-bold mb-3">{cat.title}</h3>
                            <p className="text-sm text-gray-500 mb-6 font-medium leading-relaxed">{cat.desc}</p>
                                ...
                                {cat.articles.map((article, aIdx) => (
                                    <li key={aIdx}>
                                        <a href="#" className="flex items-center gap-2 text-sm text-gray-400 hover:text-primary-600 transition-colors font-medium group/link">
                                            <span className="w-1.5 h-1.5 rounded-full bg-gray-200 group-hover/link:bg-primary-600 transition-colors" />
                                            {article}
                                        </a>
                                    </li>
                                ))}
            ...
            <section className="bg-gray-50 py-24 px-8">
                <div className="max-w-5xl mx-auto">
                    <h2 className="text-3xl font-black tracking-tight mb-12 text-center">Popular Articles</h2>
                    <div className="space-y-4">
                        {[
                            ...
                            <a key={idx} href="#" className="flex items-center justify-between p-6 bg-white rounded-2xl border border-gray-100 hover:border-primary-200 hover:shadow-md transition-all group">
                                <span className="font-bold text-gray-700 group-hover:text-primary-700 transition-colors">{article}</span>
            ...
            <section className="py-24 px-8 text-center">
                <div className="max-w-2xl mx-auto">
                    ...
                    <h2 className="text-4xl font-black tracking-tight mb-6">Still need answers?</h2>
                    <p className="text-gray-500 text-lg mb-10">
                        Our support team is available Mon-Fri, 9am-6pm EST. We usually respond within 2 hours.
                    </p>
                    <button className="bg-gray-900 text-white px-8 py-4 rounded-full text-xs font-black uppercase tracking-widest hover:bg-black transition-colors shadow-lg">
                        Contact Support
                    </button>
                </div>
            </section>
