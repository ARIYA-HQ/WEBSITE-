...
export default function FAQPage() {
    ...
    return (
        <main className="pt-24 bg-white min-h-screen">
            {/* Hero */}
            <section className="relative py-24 px-8 bg-gray-50 mb-16">
                <div className="max-w-4xl mx-auto text-center">
                    ...
                    <div className="relative max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700 delay-200">
                        <input
                            ...
                            className="w-full pl-14 pr-6 py-6 rounded-full border-none shadow-xl shadow-gray-200/50 text-lg placeholder:text-gray-400 focus:ring-2 focus:ring-primary-500 transition-all outline-none"
                        />
            ...
            <section className="max-w-4xl mx-auto px-8 mb-32">
                {faqs.map((section, sIdx) => (
                    <div key={sIdx} className="mb-16">
                        <h2 className="text-2xl font-black tracking-tight mb-8 pb-4 border-b border-gray-100">{section.category}</h2>
                                ...
                                return (
                                    <div key={qIdx} className="group">
                                        <button
                                            ...
                                            className={`w-full text-left p-6 md:p-8 rounded-[2rem] transition-all duration-300 flex justify-between items-start gap-4 ${isOpen ? 'bg-gray-50 shadow-sm' : 'hover:bg-gray-50'}`}
                                        >
                                            <span className={`text-lg md:text-xl font-bold tracking-tight transition-colors ${isOpen ? 'text-primary-600' : 'text-gray-900'}`}>
                                                {item.q}
                                            </span>
                                            <span className={`p-2 rounded-full transition-colors flex-shrink-0 ${isOpen ? 'bg-primary-100 text-primary-600' : 'bg-gray-100 text-gray-400 group-hover:bg-white group-hover:shadow-md'}`}>
                                        ...
                                        <AnimatePresence>
                                            {isOpen && (
                                                ...
                                                >
                                                    <div className="px-8 md:px-10 pb-8 text-gray-500 leading-relaxed text-lg">
                                                        {item.a}
                                                    </div>
                                                </motion.div>
                                            )}
